...
do{
  ...
  } while( wylosowanych < 12 );  //wykonuj petla tak dlugo az wylosuje 12 niepowtarzajacych sie licz
...
double chess::get_s(double f, double i_ms)   //,double fi) //, double fi)
{
    double s;

    s = 0.5*(1+sin(2*M_PI*f*(i_ms/60)));
...
int chess:: wylosuj()
{
    int temp=rand() % 12+1;     //qrand() % ((12+1)-1)+1;
 //   qDebug() << temp;
    return  temp;
}
...
void chess::timeTick()
{
    ...
    double ff =7;
    double lum = 0.5*(1+sin(2*M_PI*ff*(i_ms/60)));
    i_ms += 1;
                                    //255,255,255
    ui->label_2->setStyleSheet(QString("background-color:rgba(255,255,255,%1);").arg(lum*255));  //red    fuchsia    aqua
...
{
    double ff =7.5;
    double lum = 0.5*(1+sin(2*M_PI*ff*(i_ms1/60)));
    i_ms1 += 1;
    ui->label_3->setStyleSheet(QString("background-color:rgba(255,255,255,%1);").arg(lum*255));  //red    fuchsia    aqua
...
{
    double ff =8;
    double lum = 0.5*(1+sin(2*M_PI*ff*(i_ms2/60)));
    i_ms2 += 1;
...
{
    double ff =8.5;
    double lum =0.5*(1+sin(2*M_PI*ff*(i_ms3/60)));
     i_ms3 += 1;
                                                   //(255,255,255)
...
void chess::timeTick5()
{
        ...
        double ff = 9;
        double lum =0.5*(1+sin(2*M_PI*ff*(i_ms4/60)));
        i_ms4 += 1;
...
{
    double ff =9.5;
    double lum =sin(2*M_PI*ff*(i_ms5/60));
    i_ms5 += 1;
    ui->label_9->setStyleSheet(QString("background-color:rgba(255,255,255,%1);").arg(lum*255));  //red    fuchsia    aqua
...
void chess::timeTick7()
{
    ...
    double ff =10;
    double lum =0.5*(1+sin(2*M_PI*ff*(i_ms6/60)));
    i_ms6 += 1;
    ui->label_7->setStyleSheet(QString("background-color:rgba(255,255,255,%1);").arg(lum*255));  //red    fuchsia    aqua
...
{
    double ff =10.5;
    double lum =0.5*(1+sin(2*M_PI*ff*(i_ms7/60)));
    i_ms7 += 1;
    ui->label_6->setStyleSheet(QString("background-color:rgba(255,255,255,%1);").arg(lum*255));  //red    fuchsia    aqua
...
void chess::timeTick9()
{
    double ff =11;
    double lum =0.5*(1+sin(2*M_PI*ff*(i_ms8/60)));
    i_ms8 += 1;
...
void chess::timeTick10()
{
    ...
    double ff =11.5;
    double lum = 0.5*(1+sin(2*M_PI*ff*(i_ms9/60)));
    i_ms9 += 1;
...
{
    double ff =12;
    double lum =0.5*(1+sin(2*M_PI*ff*(i_ms10/60)));
    i_ms10 += 1;
...
void chess::timeTick12()     //15Hz
{

    double ff =12.5;  // 6.25;    //6.5;    //9Hz     13Hz
    double lum = 0.5*(1+sin(2*M_PI*ff*(i_ms11/60)));      //get_s(ff,i_ms11);  //,(1.5)*M_PI);   //, 4.71);                                             //255,255,255
      i_ms11 += 1;
...

{
    ...
    if(!start)
    {
       ...
       // qDebug() << "start";
        ...
        QString host_ip = "192.168.0.104";
        if(flag == 1)     //|| flag==5 || flag==9)
       { udpSocket->writeDatagram(ByteDataF1, QHostAddress(host_ip), 1235);  }  //12  QHostAddress:Broadcast
        else if (flag == 2) // || flag==6 || flag==10)
       { udpSocket->writeDatagram(ByteDataF2, QHostAddress(host_ip), 1235); }  //15
        else if (flag == 3) // || flag==7 || flag==11)
       { udpSocket->writeDatagram(ByteDataF3, QHostAddress(host_ip), 1235);  }  //8
        else if (flag == 4)  // || flag==8 || flag==12)
       { udpSocket->writeDatagram(ByteDataF4, QHostAddress(host_ip), 1235);}  //10
        else if (flag == 5)  // || flag==6 || flag==10)
       { udpSocket->writeDatagram(ByteDataF5, QHostAddress(host_ip), 1235); }  //15
        else if (flag == 6)  //|| flag==7 || flag==11)
       { udpSocket->writeDatagram(ByteDataF6, QHostAddress(host_ip), 1235); }  //8
        else if (flag == 7)  // || flag==8 || flag==12)
       { udpSocket->writeDatagram(ByteDataF7, QHostAddress(host_ip), 1235); }  //10
        else if (flag == 8)   //|| flag==6 || flag==10)
       { udpSocket->writeDatagram(ByteDataF8, QHostAddress(host_ip), 1235);  }  //15
        else if (flag == 9)// || flag==7 || flag==11)
       { udpSocket->writeDatagram(ByteDataF1F2, QHostAddress(host_ip), 1235); }  //8
        else if (flag == 10) // || flag==8 || flag==12)
       { udpSocket->writeDatagram(ByteDataF3F4, QHostAddress(host_ip), 1235); }  //10
        else if (flag == 11) // || flag==6 || flag==10)
       { udpSocket->writeDatagram(ByteDataF5F6, QHostAddress(host_ip), 1235); }  //15
        else if (flag == 12)  // || flag==7 || flag==11)
       { udpSocket->writeDatagram(ByteDataF7F8, QHostAddress(host_ip), 1235); }  //8
    ...
    if(shapeTime >= 236)
      {
        ...
        ui->label_10->setStyleSheet("border: transparent; ");
          ...
          else if(wylosowane[num]==2) { ui->label_3->setStyleSheet("background-color:red;");
                            flag = 2;
                           }
          else if(wylosowane[num]==3) { ui->label_4->setStyleSheet("background-color:red;");
                            flag = 3;
                            }
        ...
        //  int przerwa = 1;    //qrand() % ((7+1)-3)+3;        //przerwa losowana w [s]
        //  qDebug() << QString::number(przerwa);       //sprawdzajka
        //  QThread::msleep(1000);      //(przerwa*1000);              // opoznienie

          if(pause == 59)  //2) //  510 )  //1020)
           {
